...
\end{equation}
where $\Omega_0 = \sqrt{\Omega_{p1}\Omega_{p2}} = 0.5172
$ and $B = \Omega_{p2} - \Omega_{p1} = 0.1141$.

Substituting $\Omega_{s1}$ and $\Omega_{s2}$ in \eqref{eq:freq_transform} we obtain the stopband edges of lowpass filter 
\begin{align}
    \Omega_{Ls1} &= \frac{\Omega_{s1}^2 - \Omega_0^2}{B\Omega_{s1}} = 1.4682\\
    \Omega_{Ls2} &= \frac{\Omega_{s2}^2 - \Omega_0^2}{B\Omega_{s2}} = -1.5495
\end{align}
And we choose the minimum of these two stopband edges
\begin{align}
    \Omega_{Ls} = \mbox{min}(\vert \Omega_{Ls_1}\vert,\vert \Omega_{Ls_2}\vert) = 1.4682.
\end{align}
...
\input{tables/filter}
we get $N\geq 4$ and $0.2145 \leq \epsilon \leq 0.6197$\\
\newpage
The below code plots \eqref{eq:mag_freq_response} for different values of $\epsilon$ .
\begin{lstlisting}
https://github.com/VINAYKURREiith/Vinay1/blob/master/filter-design/codes/fig1.py
\end{lstlisting}
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/fig1.png}
\caption{The Analog Low-Pass Frequency Response for $0.2145  \leq \epsilon \leq 0.6197$}
...
\begin{lstlisting}
https://github.com/VINAYKURREiith/Vinay1/blob/master/filter-design/codes/poles.c
\end{lstlisting}
...
\begin{lstlisting}
https://github.com/VINAYKURREiith/Vinay1/blob/master/filter-design/codes/fig2.py
\end{lstlisting}
\begin{figure}[H]
\centering
...
\end{align}
Substituting respective values in \eqref{eq:Gain_eq_LP} we get $G_{LP}=2.707$
\begin{align}
     H_{a,LP}(s_L) &= \frac{2.707}{\brak{s_L-s_1}\brak{s_L-s_2}\brak{s_L-s_3}\brak{s_L-s_4}}\\
     &= \frac{2.707}{s_{L}^4 + 2.267s_{L}^3 +4.725s_{L}^2 + 4.865s_{L} +2.769}\label{eq:design}
\end{align}
\begin{figure}[H]
\centering
...
\begin{align}
    H_{a,BP}\brak{s} &= \frac{17.985s^4}{5900.074s^8 + 1526.141s^7 + 6675.925s^6 + 1267.347s^5 + 2729.986s^4 + 339.010s^3 + 477.689s^2 + 29.211s + 30.208} \label{eq:magnitude_bandpass_sdom}
\end{align}
...
\begin{align}
    H_{d,BP}(z) &= \frac{G(z^8 - 4z^6 + 6z^4 - 4z^2 + 1)}{18975.591z^8 - 82590.117z^7 + 202304.613z^6 - 319306.507z^5 + 359963.495z^4 - 288532.516z^3 + 165180.193z^2 - 60913.612z + 12652.173}
\end{align}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/fig6.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/fig7.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=1\columnwidth]{figs/fig8.png}
